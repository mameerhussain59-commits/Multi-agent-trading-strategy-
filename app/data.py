...
class HTTP:
    async def get(self,url,params=None,headers=None):
        async with httpx.AsyncClient(timeout=settings.request_timeout,follow_redirects=True) as client:
            r=await client.get(url,params=params,headers=headers); r.raise_for_status(); return r
http=HTTP()
...
async def mrnasdog_scored_tokens():
    ...
    for url in (await _research_urls())[:settings.max_research_pages]:
        try:
            r=await http.get(url); soup=BeautifulSoup(r.text,'html.parser'); text=soup.get_text(' ',strip=True)
            # Full Pressure Framework pages expose a 0-10 score; inflation-only pages expose 0-2 and are excluded.
            ...
            raw=symbol_match.group(1); symbol=raw.upper()
            if '-' in raw: symbol=raw.split('-')[0].upper()
            results.append({'symbol':symbol,'score':score,'url':url,'observed_at':now().isoformat()})
        except Exception: continue
    unique={}
    for item in results: unique[item['symbol']]=item
    return sorted(unique.values(),key=lambda x:x['score'],reverse=True)

async def dexscreener_search(query):
    r=await http.get('https://api.dexscreener.com/latest/dex/search',{'q':query}); return r.json()
async def dexscreener_token(symbol):
    data=await dexscreener_search(symbol); pairs=[p for p in (data.get('pairs') or []) if p.get('priceUsd')]
    pairs.sort(key=lambda p:float(p.get('liquidity',{}).get('usd') or 0),reverse=True); return pairs[0] if pairs else None
async def coingecko_markets(limit=250):
    ...
    r=await http.get('https://api.coingecko.com/api/v3/coins/markets',{'vs_currency':'usd','order':'market_cap_desc','per_page':min(limit,250),'page':1,'sparkline':'false','price_change_percentage':'24h'},headers=headers); return r.json()
...
async def binance_ticker(symbol):
    r=await http.get('https://api.binance.com/api/v3/ticker/24hr',{'symbol':symbol}); return r.json()
async def binance_klines(symbol,interval='1h',limit=200):
    r=await http.get('https://api.binance.com/api/v3/klines',{'symbol':symbol,'interval':interval,'limit':limit}); return r.json()
async def binance_order_book(symbol,limit=20):
    r=await http.get('https://api.binance.com/api/v3/depth',{'symbol':symbol,'limit':limit}); return r.json()
async def bybit_ticker(symbol,category='spot'):
    r=await http.get('https://api.bybit.com/v5/market/tickers',{'category':category,'symbol':symbol}); data=r.json()
    if data.get('retCode')!=0: raise DataError(data.get('retMsg','Bybit error'))
    return data
async def bybit_klines(symbol,interval='60',limit=200):
    r=await http.get('https://api.bybit.com/v5/market/kline',{'category':'spot','symbol':symbol,'interval':interval,'limit':limit}); data=r.json()
    if data.get('retCode')!=0: raise DataError(data.get('retMsg','Bybit error'))
    return data['result']['list']
async def live_market(symbol):
    pair=symbol.upper().replace('/','')+'USDT'
    try:
        t=await binance_ticker(pair); candles=await binance_klines(pair); ob=await binance_order_book(pair)
        ...
        try:
            t=await bybit_ticker(pair); item=t['result']['list'][0]; candles=await bybit_klines(pair)
        ...
        except Exception as ye: raise DataError(f'No live exchange data for {symbol}: Binance={be}; Bybit={ye}')
