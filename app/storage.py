...
def _connect():
    ...
    # Upgrade databases created by older versions without destroying data.
    for col, typ in [('exit_price','REAL'),('exit_reason','TEXT'),('pnl_usd','REAL')]:
        try: con.execute(f'ALTER TABLE paper_trades ADD COLUMN {col} {typ}')
...
def close_paper_trade(trade_id, exit_price, reason):
    with _connect() as con:
        row=con.execute('SELECT payload FROM paper_trades WHERE id=? AND status="OPEN"',(trade_id,)).fetchone()
        if not row: return None
        plan=json.loads(row[0]); qty=float(plan['position_size']); pnl=(float(exit_price)-float(plan['entry']))*qty
        con.execute('UPDATE paper_trades SET status="CLOSED",exit_price=?,exit_reason=?,pnl_usd=? WHERE id=?',(exit_price,reason,pnl,trade_id)); con.commit()
        return pnl
